---
tags: [题目/选择, 数一880, 高数/重积分及其应用]
answer: C
---
## 题干

【5】将二重积分

$$
I=\int_{\frac{\pi}{4}}^{\frac{\pi}{2}}\mathrm{d}\theta\int_0^{2\sin\theta}f(r\cos\theta,r\sin\theta)r\,\mathrm{d}r
$$

化为直角坐标系下的二次积分，则 $I=$（　）。

A. $\int_{0}^{1}dx\int_{1-\sqrt{1-x^{2}}}^{x}f(x,y)dy$

B. $\int_{0}^{1}dx\int_{x}^{\sqrt{1-x^{2}}}f(x,y)dy$

C. $\int_{0}^{1}dy\int_{0}^{y}f(x,y)dx+\int_{1}^{2}dy\int_{0}^{\sqrt{2y-y^{2}}}f(x,y)dx$

D. $\int_{0}^{1}dy\int_{y}^{\sqrt{2y-y^{2}}}f(x,y)dx$

> [!note]- 原题截图
> ![[数一880-高数-C06基选-005.png]]

> [!success]- 答案与解析
> **答案：** C
>
> **解析：** 由 $r=2\sin\theta$ 得 $x^2+y^2=2y$，结合角度范围确定区域，选 C。

## 关联

- 知识点：
- 题型：
